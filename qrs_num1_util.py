...
Original file is located at
    https://colab.research.google.com/drive/1ciaYgXWIEjUby7J3-yLkjIm6xd_FJEdp
"""
...
def EKG_QRS_detect(ecg, fs, QS, plot=False):
    ...
    if plot:
        ...
        ax.plot(R_peaks.astype(int) / fs, ecg[R_peaks.astype(int)], 'ro', label='R peaks')
        if QS:
            ax.plot(S_point.astype(int) / fs, ecg[S_point.astype(int)], 'go', label='S')
            ax.plot(Q_point.astype(int) / fs, ecg[Q_point.astype(int)], 'yo', label='Q')
